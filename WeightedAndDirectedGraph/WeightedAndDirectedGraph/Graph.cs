...
namespace WeightedAndDirectedGraph
{
    ...
    public class Graph<T>
    {
        ...
        public bool BellmanFord(Vertex<T> a)
        {
            Dictionary<Vertex<T>, VertexInfo<T>> vInfo = new()
            {
            ...
            foreach (Vertex<T> vertex in vertices)
            {
                ...
                {
                    Vertex<T> s = edge.StartVertex;
                    Vertex<T> e = edge.EndVertex;
                    vInfo.TryAdd(s, new VertexInfo<T>(s, float.PositiveInfinity, null));
                    vInfo.TryAdd(e, new VertexInfo<T>(e, float.PositiveInfinity, null));
                    if (vInfo[s].TotalCost + edge.Cost < vInfo[e].TotalCost)
                    {
                        vInfo[e] = new VertexInfo<T>(e, vInfo[s].TotalCost + edge.Cost, edge);
                    }
                }
            }
